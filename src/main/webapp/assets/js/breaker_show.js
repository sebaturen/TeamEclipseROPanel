...
function renderLastWeeks(d, diff) {
    ...
    do {
        d = getLastWoE(d, diff);
        formatDate = formatDateToAPI(d);
        formatDateShow = months[d.getMonth()] +" "+ d.getDate() +" "+ d.getFullYear();
        dContent.append("<div class='col woe_event_day day_"+ formatDate +"' data-time_break='"+ formatDate +"'>"+ formatDateShow +"</div>");
        i++;
...
function getLastWoE(d, diff) {
    ...
    return new Date(today.setDate(today.getDate()-today.getDay()+diff));
}
function formatDateToAPI(d) {
